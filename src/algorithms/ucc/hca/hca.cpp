...
unsigned long long algos::HCA::ExecuteInternal() {
    ...
    rows_data_ = std::vector<std::vector<int>>(relation_->GetNumRows(), std::vector<int>(relation_->GetNumColumns()));
    for (auto const& column_data : relation_->GetColumnData()) {
    ...
    for (size_t column_index = 0; column_index < schema_->GetNumColumns(); column_index++) {
        ...
        if (IsUnique(candidate, distinct, freq)) {
            ucc_collection_.Register(Vertical(schema_, candidate));
        } else {
            ...
            if (IsUnique(candidate, distinct_count)) {
                ...
                for (auto const& fd : fds) {
                    ...
                    if (pruning_candidate != candidate
                        ...
                        ucc_collection_.Register(Vertical(schema_, pruning_candidate));
...
bool algos::HCA::IsMinimal(boost::dynamic_bitset<> const& candidate) const {
    for (auto const& ucc : ucc_collection_.AsList()) {
        if ((candidate & ucc.GetColumnIndices())
                == ucc.GetColumnIndices()) {
            return false;
        }
    }
    return true;
...
void algos::HCA::ResetUCCAlgorithmState() {

}

void algos::HCA::LoadDataInternal(model::IDatasetStream& data_stream) {
